...
<xsl:template match="button[@id='insert-rich-text']" mode="ixsl:onclick">
	<xsl:variable name="p1">
		<xsl:for-each select="//div[@id='rich-text-preprocessor']">
			<xsl:apply-templates/>
			</xsl:for-each>
		</xsl:variable>
	...
	<xsl:result-document href="#rich-text-processed" method="ixsl:replace-content">
